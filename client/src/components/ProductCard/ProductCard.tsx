    ...
    return (
        <div className={classes.product__card}>
            <div className={classes.product__card__image__block}>
                <div className={classes.image}><img src={`${serverApi}${product.imgUrl.replace('/uploads\/', 'uploads/')}`}/></div>
                <div className={classes.weight}>
                    {product.sizes[isActiveButtonSize].value}
                    {product.category === 'drinks' ? <span>ml</span> : <span>*g</span>} 
                </div>
                {countItem > 0 && <div className={classes.weight} style={{left: '10px', width:'22px'}}>
                     <FaShoppingCart/>
                </div>}
            </div>
            <div className={classes.product__card__description}>
                <div className={classes.title}>
                    <Link className={classes.title__text} to={`products/${product._id}`}>{product.title}</Link>
                </div>
            ...
            <div className={classes.product__card__details}>
                <div className={classes.product__card__details__info}>
                    <div className={classes.sizes} 
                        ...
                        {product.sizes.map((elem, i) => (
                            <button    
                                id={`${i}`}
                                key={i}
                                className={isActiveButtonSize === i? classes.button__active : classes.button} 
                    ...
                    {product?.category === 'pizza' && <div className={classes.crust}>
                        <button 
                            id='0'  
                            className={isActiveButtonCrust === 0 ? classes.button__active : classes.button} 
                            onClick={(e) => handleClickCrust(e,0)}>
                                {typeNames[0]}
                        </button>
                        <button 
                            id='1' 
                            className={isActiveButtonCrust === 1 ? classes.button__active : classes.button}
                ...
                <div className={classes.product__card__price__row}>
                    ...
                    <div className={classes.product__card__btn__wrap}>
                        {countItem > 0 ? <div className={classes.product__card__quantity}>
                            <button onClick={onClickMinus}><AiOutlineMinus size='1.2em'/></button>
                            <p className={classes.product__card__quantity__count}>
                                {countItem < 10 && <span>0</span>}
                                {countItem}
                            </p>
                            <button onClick={onClickAddItem}><AiOutlinePlus size='1.2em'/></button>
                            </div>
                        :<button className={classes.product__card__btn_red} onClick={onClickAddItem}>To cart</button>}
                    </div> 
                </div>
            </div>
        </div>
